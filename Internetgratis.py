...
class inject(object):
    ...
    def start(self):
        try:
            ...
            frontend_domains = open(real_path('/Ro0TP00R777.txt')).readlines()
            frontend_domains = filter_array(frontend_domains)
            if len(frontend_domains) == 0:
                self.log('Frontend Domains not found. Please check Ro0TP00R777.txt', color='[Y1]')
                return
...
class domain_fronting(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.log('Ro0TP00R777 Connecting'.format(self.proxy_host, self.proxy_port))
            ...
            self.log('Ro0TP00R777 Connected ''[R2]'.format(self.proxy_host, self.proxy_port), color='[R1]')
        except OSError:
...
def main():
    print(colors('\n'.join([
        ...
        '[R1]RO0TP00R CHANNEL', '[CC]'
        '[P1]GitHub  : https://github.com/Ro0TP00R777Channel', '[CC]'
